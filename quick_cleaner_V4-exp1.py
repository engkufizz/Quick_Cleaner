...
def clean_recent_items():
    # Warning: this will reset File Explorer Quick Access pins and frequent items
...
BASE_TASKS = [
    ...
    ("Recent Items", clean_recent_items),  # Untick by default in UI to protect Quick Access pins
...
class CleanerThread(QThread):
    ...
    progress = Signal(object)       # total freed so far (bytes) - use object to avoid 32-bit int overflow
    done = Signal(object)           # total freed (bytes) - use object to avoid 32-bit int overflow
...
class QuickCleaner(QWidget):
    def __init__(self):
        ...
        # Start neutral; we'll update based on selected tasks
        ...
        # Base (Windows) task checkboxes
        self.base_checks = {}
        base_layout = QHBoxLayout()
        base_layout.setSpacing(12)
        base_title = QLabel("Windows:")
        base_title.setStyleSheet("font-size:10pt;")
        base_layout.addWidget(base_title)

        for name, _func in BASE_TASKS:
            cb = QCheckBox(name, self.panel)
            # Default all on, except 'Recent Items' off to avoid resetting Quick Access pins
            if name == "Recent Items":
                cb.setChecked(False)
                cb.setToolTip("Clearing this resets File Explorer Quick Access pins and frequent items")
            else:
                cb.setChecked(True)
            cb.stateChanged.connect(self.update_progress_max)
            self.base_checks[name] = cb
    ...
    def refresh_browser_presence(self):
        ...
        for name, cb in self.browser_checks.items():
            # Enable either way; show hint if not detected
            cb.setEnabled(True)
            if not present.get(name, False):
                cb.setToolTip("No data found; cleaning may have no effect")
            else:
                cb.setToolTip("")
        # Progress max will be updated by update_progress_max hook

    def update_progress_max(self):
        selected_base = sum(1 for cb in self.base_checks.values() if cb.isChecked())
        selected_browsers = sum(1 for cb in self.browser_checks.values() if cb.isChecked())
        total = selected_base + selected_browsers
    ...
    def set_busy(self, busy: bool):
        ...
        for cb in self.base_checks.values():
    ...
    def build_task_list(self):
        tasks = []
        # Include selected base tasks
        for name, func in BASE_TASKS:
            if self.base_checks.get(name) and self.base_checks[name].isChecked():
                tasks.append((name, func))
        # Include selected browser tasks
        for name, cb in self.browser_checks.items():
            if cb.isChecked():
    ...
    def start_clean(self):
        ...
        self.progress.setRange(0, len(tasks) if len(tasks) > 0 else 1)
        self.progress.setValue(0)
    ...
    def on_progress(self, total_bytes):
        self.total_label.setText(f"Freed: {human_size(int(total_bytes))}")
