...
from PySide6.QtWidgets import (
    ...
    QGraphicsDropShadowEffect, QSystemTrayIcon, QMenu, QProgressBar
)

# ---------------------- Utilities ----------------------
def human_size(n):
    # No percent symbols - avoids any locale text issues
    units = ["B", "KB", "MB", "GB", "TB"]
    i = 0
    f = float(n)
...
def safe_remove_file(p: Path) -> int:
    try:
        if not p.exists() or p.is_symlink():
            # Skip symlinks to be extra safe
            return 0
        size = p.stat().st_size
        try:
...
def wipe_tree(path: Path) -> int:
    # Delete a directory tree, returning bytes freed. Skips symlinks.
    freed = 0
    try:
...
def empty_recycle_bin() -> int:
    # Returns bytes freed (size before empty)
    info = SHQUERYRBINFO()
    info.cbSize = ctypes.sizeof(SHQUERYRBINFO)
    try:
        res = SHQueryRecycleBinW(None, ctypes.byref(info))
        before = info.i64Size if res == 0 else 0
    except Exception:
        before = 0
    try:
...
def clean_windows_temp():
    # Works best when elevated; without admin it will still clean what it can
    return wipe_dir_contents(WINDIR / "Temp")
...
def clean_thumbnails():
    # Thumb/icon caches – safe; Explorer rebuilds them
...
def clean_chromium_user_data(root: Path) -> int:
    ...
    # Profiles can be "Default", "Profile N", "Guest Profile", etc.
    try:
        candidates = [root]
...
def clean_opera():
    # Opera profiles are directly under "Opera Stable"
    root = LOCALAPPDATA / "Opera Software" / "Opera Stable"
    freed = 0
    if root.exists():
        rels = ["Cache", "Code Cache", "GPUCache", "ShaderCache", "DawnCache", "Media Cache", os.path.join("Service Worker", "CacheStorage")]
        for rel in rels:
...
CLEAN_TASKS = [
    ...
    ("Google Chrome Cache", clean_chrome),
    ("Microsoft Edge Cache", clean_edge),
    ("Brave Cache", clean_brave),
    ("Vivaldi Cache", clean_vivaldi),
    ("Opera Cache", clean_opera),
    ("Firefox Cache", clean_firefox),
    # ("Windows Temp", clean_windows_temp),  # Optional; comment in if you want best-effort Windows Temp
]
...
class CleanerThread(QThread):
    ...
    def run(self):
        total_freed = 0
        steps = len(CLEAN_TASKS)
        gc.collect()
        for i, (name, func) in enumerate(CLEAN_TASKS, start=1):
...
class QuickCleaner(QWidget):
    def __init__(self):
        ...
        self.progress.setRange(0, len(CLEAN_TASKS))
        self.progress.setValue(0)
        self.progress.setTextVisible(False)

        # Buttons on the side
        self.more_btn = QToolButton(self.panel)
        self.more_btn.setText("➜")
        self.more_btn.setToolTip("Close")
        self.more_btn.clicked.connect(QApplication.instance().quit)
...
def main():
    ...
    app.setLayoutDirection(Qt.LeftToRight)  # keep labels simple
